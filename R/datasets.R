...
datasets.contrasts <- function () {
    ...
    colnames (designs[[2]]) <- c ("MGd_4h", "ctrl_4h", "MGd_12h", "ctrl_12h",
                                  "MGd_24h", "ctrl_24h")
    colnames (designs[[3]]) <- sapply (c (10:14, 1:9),
                                       function (i) paste ("exp", i, sep = "_"))
    contrasts <- list (makeContrasts (MEM - CYT, levels = designs[[1]]),
                       makeContrasts (MGd_4h - ctrl_4h, MGd_12h - ctrl_12h,
                                      MGd_24h - ctrl_24h, levels = designs[[2]]),
                       makeContrasts (exp_1 - exp_2, exp_2 - exp_3, exp_3 - exp_4,
                                      exp_11 - exp_12, exp_12 - exp_13, exp_13 - exp_14,
                                      levels = designs[[3]]))
}
